...
public function buildTasksForImport($executionID)
{
    ...
    foreach($data->name as $key => $name)
    {
        ...
        foreach($extendFields as $extendField)
        {
            $dataArray = zget($_POST, $extendField->field, array());
            $taskData->{$extendField->field} = zget($dataArray, $key, '');
            ...
            if(empty($taskData->{$extendField->field}) && ($extendField->type == 'decimal'  || $extendField->type == 'int'))  $taskData->{$extendField->field} = 0;
            if(empty($taskData->{$extendField->field}) && ($extendField->type == 'datetime' || $extendField->type == 'date')) $taskData->{$extendField->field} = NULL;
        }

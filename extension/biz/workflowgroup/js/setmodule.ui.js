...
window.checkRelated = function(id, type)
{
    if(type === 'open')
    {
        ...
        else if(id == 'feature-auditplan')
        {
            if(!isModuleEnabled('feature-process'))
            {
                const message = openDependFeature.replace('{source}', auditplanLang).replace('{target}', processLang);
                showDependencyConfirm(
                    message,
                    () => setModuleState('feature-process', true),
                    () => setModuleState('feature-auditplan', false)
                );
                return false;
            }
        }
    ...
    else
    {
        ...
        else if(id == 'feature-process')
        {
            if(isModuleEnabled('feature-auditplan'))
            {

...
class workflowdatasourceModel extends model
{
    ...
    public function fix($datasource)
    {
        ...
        elseif($datasource->type == 'system')
        {
            ...
            if(!isset($this->config->workflowdatasource->methods[$datasource->module]))
            {
                $errors['module'] = $this->lang->workflowdatasource->error->invalidModule;
            }
            elseif(!in_array($datasource->method, $this->config->workflowdatasource->methods[$datasource->module]))
            {
                $errors['method'] = $this->lang->workflowdatasource->error->invalidMethod;
            }
    ...
    public function delete($id, $null = null)
    {
        try
        {
            ...
            $this->dbh->query($sql);
            $this->loadModel('workflow', 'flow')->logChangeSQL($sql);
            $this->dao->delete()->from(TABLE_WORKFLOWDATASOURCE)->where('id')->eq($id)->exec();
            if(!dao::isError()) $this->loadModel('workflow', 'flow')->logChangeSQL($this->dao->get());
        }
        catch(PDOException $exception)
        {
            dao::$errors = $exception->getMessage();

...
public function createRelation($relations)
{
    ...
    $issueLink    = $this->getJiraData($this->session->jiraMethod == 'api' ? 'json' : $this->session->jiraMethod, 'issuelinktype');
...
public function importConfluenceSpace($dataList)
{
    ...
    foreach($dataList as $space)
    {
        ...
        $lib->addedBy   = substr($lib->addedBy, 0, 30);
...
public function importConfluenceFolder($dataList)
{
    ...
    foreach($dataList as $libID => $folderList)
    {
        ...
        foreach($folderList as $folder)
        {
            ...
            $this->dao->update(TABLE_DOC)->set('path')->eq(",{$docID},")->where('id')->eq($docID)->exec();
    ...
    foreach(array('folder', 'page', 'embed', 'database', 'whiteboard') as $objectType)
    {
        ...
        if(!empty($result->results))
        {
            ...
            foreach($result->results as $object)
            {
                ...
                $path = ",{$docID},";
                if($parent) $path = $this->dao->select('id,path')->from(TABLE_DOC)->where('id')->eq($parent)->fetch('path') . "{$docID},";
                $this->dao->update(TABLE_DOC)->set('path')->eq($path)->where('id')->eq($docID)->exec();
                $this->action->create('doc', $docID, 'Created');
...
public function importConfluenceDraftPage($dataList)
{
    ...
    foreach($dataList as $page)
    {
        ...
        $this->dao->insert(TABLE_DOC)->data($doc)->exec();
        $docID = $this->dao->lastInsertID();
        $this->dao->update(TABLE_DOC)->set('path')->eq(empty($parentDoc) ? ",{$docID}," : $parentDoc->path . "{$docID},")->where('id')->eq($docID)->exec();
 ...
 * @return array
 */
public function getConfluenceData($module, $group = null, $start = 0, $next = '')
{
    ...
    {
        $url    = $confluenceApi['domain'] . ($next ?: "/rest/api/content/search?cql=type=comment&start={$start}&limit=1000&expand=body.storage,history,container");
        $result = json_decode(commonModel::http($url, array(), array(), array("Authorization: Basic $token"), 'json', 'GET', 10));
        ...
        if(!empty($result->results))
        {
            ...
            $links = '_links';
            if(!empty($result->{$links}->next)) $comments = array_merge($comments, $this->getConfluenceData($module, $group, 0, $result->{$links}->next));
        }
    ...
    {
        $url    = $confluenceApi['domain'] . ($next ?: "/rest/api/content/search?cql=type=attachment&start={$start}&limit=1000&expand=container,history");
        $result = json_decode(commonModel::http($url, array(), array(), array("Authorization: Basic $token"), 'json', 'GET', 10));
        ...
        if(!empty($result->results))
        {
            ...
            $links = '_links';
            if(!empty($result->{$links}->next)) $attachments = array_merge($attachments, $this->getConfluenceData($module, $group, 0, $result->{$links}->next));
        }
        return $attachments;
    }

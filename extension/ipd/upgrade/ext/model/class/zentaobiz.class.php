...
class zentaobizUpgrade extends bizextUpgrade
{
    ...
    public function importBuildinModules($vision = 'all')
    {
        ...
        foreach($modules as $app => $appModules)
        {
            ...
            foreach($appModules as $module => $options)
            {
                ...
                $flow = $this->dao->select('id')->from(TABLE_WORKFLOW)->where('app')->eq($app)->andWhere('module')->eq($module)->andWhere('vision')->eq($data->vision)->fetch();
                if(empty($flow))
                {
        ...
        foreach($actions as $module => $moduleActions)
        {
            ...
            foreach($moduleActions as $action)
            {
                ...
                $action = $this->dao->select('id')->from(TABLE_WORKFLOWACTION)->where('module')->eq($module)->andWhere('action')->eq($action)->andWhere('vision')->eq($data->vision)->fetch();
                if(empty($action))
                {
        ...
        foreach($fields as $module => $moduleFields)
        {
            ...
            foreach($moduleFields as $field => $options)
            {
                ...
                $field = $this->dao->select('id')->from(TABLE_WORKFLOWFIELD)->where('module')->eq($module)->andWhere('field')->eq($field)->fetch();
                if(empty($field))
                {
    ...
    public function addReportActions()
    {
        ...
        foreach($flows as $module)
        {
            ...
            $action->module = $module;
            $this->dao->delete()->from(TABLE_WORKFLOWACTION)->where('module')->eq($module)->andWhere('action')->eq($actionCode)->exec();
            $this->dao->insert(TABLE_WORKFLOWACTION)->data($action)->autoCheck()->exec();
        }
        return !dao::isError();
    }

...
public function importToLib($docIdList = array(), $assetType = 'practice')
{
    ...
    foreach($docs as $doc)
    {
        ...
        $assetDoc->parent       = $doc->parent;
        $assetDoc->path         = $doc->path;
        $assetDoc->lib          = 0;
        $assetDoc->version      = 1;
        $assetDoc->acl          = 'open';
        ...
        $idMap[$doc->docID]   = $assetDocID;
        $newDocs[$assetDocID] = $assetDoc;
    ...
    /* 将新文档的父级和path更新成新的ID. */
    foreach($newDocs as $assetDocID => $assetDoc)
    {
        $newPath   = [];
        $newParent = isset($idMap[$assetDoc->parent]) ? $idMap[$assetDoc->parent] : 0;
        foreach(explode(',', $assetDoc->path) as $path)
        {
            if(isset($idMap[$path])) $newPath[] = $idMap[$path];
        }
        $newPath = implode(',', $newPath);

        $this->dao->update(TABLE_DOC)->set('parent')->eq($newParent)->set('path')->eq($newPath)->where('id')->eq($assetDocID)->exec();
    }

...
elseif(empty($maxImport) and $allCount > $this->config->file->maxImport)
{
    ...
    (
        ...
        html(sprintf($lang->file->importSummary, $allCount, html::input('maxImport', $config->file->maxImport, "class='form-control' style='width:50px;display:inline;margin:0 5px;height:25px'"), ceil($allCount / $config->file->maxImport))),
        btn(setID('import'), setClass('primary', 'ml-3'), set::size('sm'), on::click('setMaxImport'), $lang->import)
    );
}
else
{

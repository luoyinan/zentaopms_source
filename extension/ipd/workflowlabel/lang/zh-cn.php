<?php
$lang->workflowlabel->common   = '工作流标签';
$lang->workflowlabel->browse   = '所有标签';
$lang->workflowlabel->create   = '新增标签';
$lang->workflowlabel->edit     = '编辑标签';
$lang->workflowlabel->delete   = '删除标签';
$lang->workflowlabel->sort     = '标签排序';
$lang->workflowlabel->search   = '搜索';
$lang->workflowlabel->settings = '标签及属性设置';

$lang->workflowlabel->id          = '编号';
$lang->workflowlabel->module      = '所属模块';
$lang->workflowlabel->label       = '检索标签';
$lang->workflowlabel->params      = '检索条件';
$lang->workflowlabel->type        = '条件类型';
$lang->workflowlabel->sql         = 'SQL条件';
$lang->workflowlabel->order       = '顺序';
$lang->workflowlabel->orderBy     = '数据排序';
$lang->workflowlabel->buildin     = '内置';
$lang->workflowlabel->createdBy   = '由谁创建';
$lang->workflowlabel->createdDate = '创建日期';
$lang->workflowlabel->editedBy    = '由谁编辑';
$lang->workflowlabel->editedDate  = '编辑日期';

$lang->workflowlabel->operatorList['equal']      = '=';
$lang->workflowlabel->operatorList['notequal']   = '!=';
$lang->workflowlabel->operatorList['gt']         = '>';
$lang->workflowlabel->operatorList['ge']         = '>=';
$lang->workflowlabel->operatorList['lt']         = '<';
$lang->workflowlabel->operatorList['le']         = '<=';
$lang->workflowlabel->operatorList['include']    = '包含';
$lang->workflowlabel->operatorList['notinclude'] = '不包含';
$lang->workflowlabel->operatorList['between']    = '介于';

$lang->workflowlabel->typeList['data'] = '以数据定义条件';
$lang->workflowlabel->typeList['sql']  = '以SQL语句定义条件';

$lang->workflowlabel->orderTypeList['asc']  = '正序';
$lang->workflowlabel->orderTypeList['desc'] = '倒序';

$lang->workflowlabel->buildinList['0'] = '否';
$lang->workflowlabel->buildinList['1'] = '是';

$lang->workflowlabel->confirmDelete = '您确定要执行删除操作吗？';

$lang->workflowlabel->default = new stdclass();
$lang->workflowlabel->default->labels['all'] = '全部';

$lang->workflowlabel->approval = new stdclass();
$lang->workflowlabel->approval->labels['review']     = '待审批';
$lang->workflowlabel->approval->labels['reviewedby'] = '我评审';

$lang->workflowlabel->error = new stdclass();
$lang->workflowlabel->error->emptyParams = '检索条件不能为空！';
$lang->workflowlabel->error->emptySQL    = '请输入SQL条件语句';
$lang->workflowlabel->error->unsafeSQL   = 'SQL条件包含不允许使用的语法，请检查后重试。';
$lang->workflowlabel->error->invalidSQL  = 'SQL条件无效，可能是字段名已发生修改，或SQL语句不合法，请检查后重试。';

$lang->workflowlabel->placeholder = new stdclass();
$lang->workflowlabel->placeholder->sql = '请输入SQL条件语句，例如：status = "doing"';

$lang->workflowlabel->tips = new stdclass();
$lang->workflowlabel->tips->known    = '知道了';
$lang->workflowlabel->tips->features = '在流程的列表页可以通过标签浏览不同的数据。';

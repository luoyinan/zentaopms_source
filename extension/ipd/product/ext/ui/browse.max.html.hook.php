...
$importItems = array();
if($canBeChanged && hasPriv($moduleName, 'import') && empty(data('hasFrozenStories'))) $importItems[] = array('text' => $lang->story->import, 'data-toggle' => 'modal', 'data-size' => 'sm', 'url' => $importLink);
if($canBeChanged && $canImportFromLib && $app->tab == 'project' && empty(data('hasFrozenStories'))) $importItems[] = array('text' => $lang->projectstory->importFromLib, 'url' => $importFromLibLink);
...
query('#exportBtn')->replaceWith(
    ...
    $canExport && !$canExportTemplate ? btn(setID('exportBtn'), setClass('toolbar-item ghost btn btn-default'), set::url($exportLink),         set::icon('export'), setData('toggle', 'modal'), setData('size', 'sm'), $lang->export) : null,

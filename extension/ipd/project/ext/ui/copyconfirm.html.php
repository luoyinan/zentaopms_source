...
if(in_array($project->model, array('scrum', 'agileplus', 'kanban')))
{
    ...
    if($config->vision != 'rnd') $items['days'] = array('name' => 'days', 'label' => $lang->execution->days, 'control' => 'input', 'width' => '100px', 'hidden' => !empty($copyProject->isTpl));
    if($project->model != 'kanban') $items['lifetime'] = array('name' => 'lifetime', 'label' => $lang->execution->type, 'control' => array('control' => 'picker', 'required' => true), 'items' => $lang->execution->lifeTimeList, 'width' => '80px');
...
else
{
    ...
    if(!empty($executionIdList))
    {
        ...
        $items['attribute'] = array('name' => 'attribute', 'label' => $lang->programplan->attribute, 'control' => array('control' => 'picker', 'required' => true), 'width' => '80px', 'items' => $typeList, 'hidden' => $project->model == 'ipd');
        $items['acl']       = array('name' => 'acl', 'label' => $lang->programplan->acl, 'control' => array('control' => 'picker', 'required' => true), 'width' => '80px', 'items' => $lang->execution->aclList);

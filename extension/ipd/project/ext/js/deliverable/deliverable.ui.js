    ...
    if(result[0] && col.name == 'actions')
    {
        if (typeof result[0]?.props?.items == 'object')
        {
            result[0].props.items.map(item => {
                ...
                /* 已经通过的交付物再建评审要从头发起。*/
                if(item.icon == 'sub-review' && row.data?.submitNoReviewID)
                {
                    item.url = $.createLink('review', 'submit', `reviewID=0&from=deliverable&extra=projectID=${row.data.project},deliverableID=${row.data.id},objectID=${row.data.deliverable}`);
                }

...
$basicItems[$lang->review->status]      = zget($lang->review->statusList, $review->status) . ($review->isConditionalPassApproval ? '(' . $lang->review->resultList['conditional'] . ')' : '');
...
{
    $reviewclGroup = array();
    foreach($reviewcl as $list) $reviewclGroup[$list->category][] = $list;

    $reviewclHtml  = "<table class='table bordered condensed reviewcl'>";
    $reviewclHtml .= "<thead><tr><th class='w-1/2'>{$lang->review->listItem}</th></tr></thead>";
    $reviewclHtml .= '<tbody>';
    foreach($reviewclGroup as $category => $lists)
    {
        ...
        $reviewclHtml .= "<tr><td class='text-left font-bold' id='category-{$category}'>" . zget($categoryList, $category) . $btn . '</td></tr>';
    ...
    $tabs[] = setting()
        ->group('basic')
        ->title($lang->review->reviewcl)
        ->control('html')
        ->content($reviewclHtml);
}
...
if(!empty($reviewIssues))
{
    ...
    $issueCols['title']['fixed']   = false;
    foreach($issueCols as $colName => $issueCol) $issueCols[$colName]['sortType'] = false;
    unset($issueCols['listID']);

    /* 问题列表。*/
    $tabs[] = setting()
        ->group('basic')
        ->title($lang->review->issueList)
        ->control('dtable')
        ->cols($issueCols)
        ->data(array_values($reviewIssues));
}

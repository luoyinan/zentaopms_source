...
protected function buildSearchForm($objectID, $objectType, $relatedObjectType)
{
    ...
    {
        $module = str_replace('workflow_', '', $relatedObjectType);
        $flow   = $this->loadModel('workflow')->getByModule($module);
        $this->loadModel('flow')->setSearchParams($flow, null, $actionURL, $relatedObjectType);
    }
    if($relatedObjectType == 'projectdeliverable')
    {
        $this->loadModel('project');
        $this->app->loadLang('review');
        $this->app->loadLang('deliverable');

        $projectDeliverableSearch['actionURL'] = $actionURL;
        $projectDeliverableSearch['queryID']   = 0;
        $projectDeliverableSearch['module']    = $relatedObjectType;
        $projectDeliverableSearch['fields']['project']       = $this->lang->project->deliverableProject;
        $projectDeliverableSearch['fields']['name']          = $this->lang->deliverable->title;
        $projectDeliverableSearch['fields']['version']       = $this->lang->deliverable->version;
        $projectDeliverableSearch['fields']['status']        = $this->lang->deliverable->reviewStatus;
        $projectDeliverableSearch['fields']['submittedBy']   = $this->lang->project->submitedBy;
        $projectDeliverableSearch['fields']['submittedDate'] = $this->lang->project->submitedDate;
        $projectDeliverableSearch['fields']['id']            = $this->lang->idAB;
        $projectDeliverableSearch['params']['project']       = array('operator' => '=', 'control' => 'select', 'values' => $this->project->getPairs());
        $projectDeliverableSearch['params']['name']          = array('operator' => 'include', 'control' => 'input', 'values' => '');
        $projectDeliverableSearch['params']['version']       = array('operator' => 'include', 'control' => 'input', 'values' => '');
        $projectDeliverableSearch['params']['status']        = array('operator' => '=', 'control' => 'select', 'values' => $this->lang->review->statusList);
        $projectDeliverableSearch['params']['submittedBy']   = array('operator' => '=', 'control' => 'select', 'values' => 'users');
        $projectDeliverableSearch['params']['submittedDate'] = array('operator' => '=', 'control' => 'date', 'values' => '');
        $projectDeliverableSearch['params']['id']            = array('operator' => '=', 'control' => 'input', 'values' => '');

        unset($projectDeliverableSearch['params']['status']['values']['done']);
        unset($projectDeliverableSearch['params']['status']['values']['reverting']);
        $projectDeliverableSearch['params']['status']['values']['draft'] .= '(' . $this->lang->project->needConfirm . ')';
        $projectDeliverableSearch['params']['status']['values']['pass']  .= '(' . $this->lang->project->confirmed . ')';

        $this->loadModel('search')->setSearchParams($projectDeliverableSearch);
    }
}
...
protected function getGraphData($relatedObjects, $objectID, $objectType)
{
    ...
    if(strpos($objectType, 'workflow_') === false && $objectType != 'doc' && $objectType != 'projectdeliverable')
    {
    ...
    foreach($relatedObjects as $relationKey => $objectList)
    {
        ...
        foreach($objectList as $objectType => $objectInfo)
        {
            ...
            foreach($objectInfo as $objectID => $object)
            {
                ...
                if(strpos($objectType, 'workflow_') === false && !in_array($objectType, array('release', 'build', 'ppm', 'doc', 'projectdeliverable')))
                {

...
jsVar('pageParams',   "mode=$mode&status=&app={$currentApp}&param={$param}");
...
(
    ...
    'active'    => !$currentApp,
    'url'       => inlink('browseFlow', "mode=$mode&status=&app=&param={$param}&onlyCustom={$onlyCustom}&orderBy=$orderBy"),
...
foreach($flowApps as $appCode)
{
    ...
    (
        'text'      => $appName,
        'active'    => $isActive,
        'url'       => inlink('browseFlow', "mode=browse&status=&app=$appCode&param={$param}&onlyCustom={$onlyCustom}&orderBy=$orderBy"),
...
(
    ...
    (
        setClass('mx-2'),
        checkbox(setID('onlyCustom'), set::text($lang->workflow->onlyCustom), set::checked(!empty($onlyCustom)), setData(array('on' => 'change', 'call' => 'changeItem')))
    ),
);

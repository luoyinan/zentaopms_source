...
{
    if($deliverable->isConditionalPassApproval) $status .= '(' . $lang->review->resultList['conditional'] . ')';
}

$moduleName = zget($modules, $deliverable->module, '');
$moduleName = ltrim($moduleName, '/');
$basicItems = array();
if(helper::hasFeature('project_process') && $hasProcessFeature) $basicItems[$lang->deliverable->activity] = array('control' => 'html', 'content' => zget($activities, $deliverable->activity, ''), 'title' => zget($activities, $deliverable->activity, ''));
...
$basicItems[$lang->deliverable->submitedBy]    = !empty($deliverable->submittedBy) ? zget($users, $deliverable->submittedBy) . $lang->at . $deliverable->submittedDate :  zget($users, $deliverable->createdBy) . $lang->at . $deliverable->createdDate;
...
foreach($actions as $id => $action)
{
    ...
    {
        if(!empty($deliverable->submitNoReviewID))
        {
            $actions[$id]['url'] = helper::createLink('review', 'submit', "reviewID=0&from=deliverable&extra=projectID={$deliverable->project},deliverableID={$deliverable->id},objectID={$deliverable->deliverable}");
        }
        ...
        if($deliverable->hasApproval == 0)
        {
            ...
            $actions[$id]['url']  = helper::createLink('review', 'create', "projectID={$deliverable->project}&deliverable={$deliverable->id}&reviewID=0&type=deliverable&objectID=0&from=deliverable");
        }
    }

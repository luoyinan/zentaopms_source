...
window.waitDom('input[name=begin], input[name=end]', function()
{
    const begin = $('form input[name=begin]').val();
    const end   = $('form input[name=end]').val();
    if(begin && end && $('form .btn-calendar').hasClass('disabled'))
    {
        $('form .btn-calendar').removeClass('disabled');
        $('form input[name=end]').trigger('change');
        computeWorkDays();
    }
});

 ...
 * @return string
 */
public function getAfterCreateLocation($productID, $branch, $objectID, $storyID, $storyType,  $planID, $extra = '')
{
    $extra = str_replace(array(',', ' '), array('&', ''), $extra);
    parse_str($extra, $output);
    extract($output);

    if(empty($fromType)) return parent::getAfterCreateLocation($productID, $branch, $objectID, $storyID, $storyType, $planID, $extra);

...
if(!empty($hasDeliverableFeature))
{
    $deliverableItems = array();
    foreach($deliverables as $deliverable)
    {
        $deliverableItem = new stdclass();
        $deliverableItem->id    = $deliverable->id;
        $deliverableItem->title = $deliverable->name;

        $deliverableItems[] = $deliverableItem;
    }
    $tabs[] = setting()
        ->group('basic')
        ->title($lang->activity->deliverableTypes)
        ->control('entitylist')
        ->items($deliverableItems)
        ->viewUrl(createLink('deliverable', 'view', "id={id}"));
}
